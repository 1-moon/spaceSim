...
{
	float x = (aLeft.v[0] * aRight.x) + (aLeft.v[1] * aRight.y) + (aLeft.v[2] * aRight.z);
	float y = (aLeft.v[3] * aRight.x) + (aLeft.v[4] * aRight.y) + (aLeft.v[5] * aRight.z);
	float z = (aLeft.v[6] * aRight.x) + (aLeft.v[7] * aRight.y) + (aLeft.v[8] * aRight.z);
	return { x, y, z };
}
